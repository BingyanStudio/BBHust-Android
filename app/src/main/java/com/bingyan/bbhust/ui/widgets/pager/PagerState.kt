package com.bingyan.bbhust.ui.widgets.pager
...
/**
 * A state object that can be hoisted to control and observe scrolling for [AppHorizontalPager].
 *
...
class PagerState(
    ...
    private val scrollableState = ScrollableState { deltaPixels ->
        if (DebugLog) {
            Log.d("PagerState", "ScrollableState onScroll $deltaPixels")
        }
    ...
    var pageCount: Int
        ...
        set(@IntRange(from = 0) value) {
            ...
            if (value != _pageCount) {
                _pageCount = value
                if (DebugLog) {
                    Log.d("PagerState", "Page count changed: $value")
                }
    ...
    var currentPage: Int
        ...
        private set(value) {
            ...
            if (moddedValue != _currentPage) {
                ...
                if (DebugLog) {
                    Log.d("PagerState", "Current page changed: $_currentPage")
                }
     ...
     */
    fun snapToPage(page: Int, offset: Float = 0f) {
        if (DebugLog) {
            Log.d(
                "PagerState", "snapToPage. page:$currentLayoutPage, offset:$currentLayoutPageOffset"
            )
        }
        // Snap the layout
    ...
    private suspend fun animateToPageSkip(
        ...
        animate(
            ...
            if (DebugLog) {
                Log.d("PagerState", "animateToPageSkip: $layoutPages")
            }
    ...
    private fun updateLayoutPages(page: Int) {
        requireCurrentPage(page, "page")
        if (DebugLog) Log.d("PagerState", "updateLayoutPages(page = $page)")
    ...
    internal suspend fun fling(
        ...
        if (DebugLog) {
            Log.d(
                "PagerState", "fling. velocity:%.4f, page: %d, offset:%.4f, targetOffset:%.4f"
                    .format(
                        initialVelocity,
                        currentLayoutPage,
        ...
        if (targetOffset.absoluteValue >= 1) {
            ...
            AnimationState(
                ...
                if (DebugLog) {
                    Log.d(
                        "PagerState", "fling. decay. value:%.4f, page: %d, offset:%.4f"
